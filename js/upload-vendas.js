...
function generateWeekFields() {
  ...
  weeks.forEach((week, index) => {
    ...
    button.addEventListener("click", () =>
      uploadFile(fileInput, index + 1, weekDiv)
    );
...
async function uploadFile(fileInput, weekIndex, weekDiv) {
  ...
  try {
    const response = await fetch(
      `http://172.16.222.65:8080/atendentes/upload/${weeks[
        weekIndex - 1
      ].toLowerCase()}-semana/${storeId}`,
      {
        method: "PATCH",
        body: formData,
      }
...
async function zerarValoresAtendentes() {
  ...
  try {
    const response = await fetch(`http://172.16.222.65:8080/lojas/${storeId}`, {
      method: "PATCH",
    });
    ...
    if (response.ok) {
      ...
      successMessage.textContent =
        "Valores dos atendentes zerados com sucesso!";
