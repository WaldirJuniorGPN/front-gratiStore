document.addEventListener("DOMContentLoaded", () => {
  carregarLojas();
  document
    .getElementById("formCadastroLoja")
    .addEventListener("submit", cadastrarLoja);
});
...
async function cadastrarLoja(event) {
  ...
  try {
    const response = await fetch("http://172.16.222.65:8080/lojas", {
      method: "POST",
      headers: {
...
async function excluirLoja(id) {
  ...
  try {
    const response = await fetch(`http://172.16.222.65:8080/lojas/${id}`, {
      method: "DELETE",
    });
