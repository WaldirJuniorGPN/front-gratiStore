const API_BASE_URL = "http://172.16.222.65:8080";
...
async function carregarHistorico(atendenteId, ano, mes) {
  try {
    ...
    if (resp.ok) {
      ...
      return lista.filter(
        (r) =>
          r.atendenteId === atendenteId && r.data.startsWith(`${ano}-${mes}`)
      );
    }
  } catch (err) {
...
function criarCardDia(dataIso, registro) {
  ...
  card.innerHTML = `
        ...
        <input type="time" class="inicio-almoco" value="${
          registro?.inicioAlmoco || ""
        }">
        <input type="time" class="fim-almoco" value="${
          registro?.fimAlmoco || ""
        }">
  ...
  if (registro && registro.id) {
    ...
    btnAtualizar.addEventListener("click", () =>
      atualizarPonto(registro.id, card)
    );
