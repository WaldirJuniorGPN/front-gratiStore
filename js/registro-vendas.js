...
async function saveEmployeeData(employee) {
  const data = {
    vendasPrimeiraSemana:
      parseFloat(employee.vendasSemana1.replace(",", ".")) || 0.0,
    atrasoPrimeiraSemana: employee.atrasoSemana1 || "nao",
    quantidadeAtendimentosPrimeiraSemana:
      parseInt(employee.atendimentosSemana1) || 0,

    vendasSegundaSemana:
      parseFloat(employee.vendasSemana2.replace(",", ".")) || 0.0,
    atrasoSegundaSemana: employee.atrasoSemana2 || "nao",
    quantidadeAtendimentosSegundaSemana:
      parseInt(employee.atendimentosSemana2) || 0,

    vendasTerceiraSemana:
      parseFloat(employee.vendasSemana3.replace(",", ".")) || 0.0,
    atrasoTerceiraSemana: employee.atrasoSemana3 || "nao",
    quantidadeAtendimentosTerceiraSemana:
      parseInt(employee.atendimentosSemana3) || 0,

    vendasQuartaSemana:
      parseFloat(employee.vendasSemana4.replace(",", ".")) || 0.0,
    atrasoQuartaSemana: employee.atrasoSemana4 || "nao",
    quantidadeAtendimentosQuartaSemana:
      parseInt(employee.atendimentosSemana4) || 0,

    vendasQuintaSemana:
      parseFloat(employee.vendasSemana5.replace(",", ".")) || 0.0,
    atrasoQuintaSemana: employee.atrasoSemana5 || "nao",
    quantidadeAtendimentosQuintaSemana:
      parseInt(employee.atendimentosSemana5) || 0,

    vendasSextaSemana:
      parseFloat(employee.vendasSemana6.replace(",", ".")) || 0.0,
    atrasoSextaSemana: employee.atrasoSemana6 || "nao",
    quantidadeAtendimentosSextaSemana:
      parseInt(employee.atendimentosSemana6) || 0,
  };

  try {
    const response = await fetch(
      `http://172.16.222.65:8080/atendentes/${employee.id}`,
      {
        method: "PATCH",
        headers: {
...
function updateArrows() {
  ...
  rightArrow.style.display =
    visibleWeeks[visibleWeeks.length - 1] < totalWeeks ? "block" : "none";
}
...
async function fetchStores() {
  try {
    const response = await fetch("http://172.16.222.65:8080/lojas/listar");
    if (response.ok) {
...
async function fetchEmployeesByStore(storeId) {
  try {
    const response = await fetch(
      `http://172.16.222.65:8080/lojas/${storeId}/atendentes`
    );
    if (response.ok) {
