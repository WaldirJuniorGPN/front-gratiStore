...
async function atualizarCalculadora() {
  ...
  const percentualPrimeiroColocado = parseFloat(
    document.getElementById("percentualPrimeiroColocado").value
  );
  const percentualSegundoColocado = parseFloat(
    document.getElementById("percentualSegundoColocado").value
  );
  const percentualTerceiroColocado = parseFloat(
    document.getElementById("percentualTerceiroColocado").value
  );
  const percentualDemaisColocados = parseFloat(
    document.getElementById("percentualDemaisColocados").value
  );
  const bonusPrimeiroColocado = parseFloat(
    document.getElementById("bonusPrimeiroColocado").value
  );
  const bonusSegundoColocado = parseFloat(
    document.getElementById("bonusSegundoColocado").value
  );
  const bonusTerceiroColocado = parseFloat(
    document.getElementById("bonusTerceiroColocado").value
  );
...
async function carregarLojas() {
  try {
    const response = await fetch("http://172.16.222.65:8080/lojas/listar"); // Ajuste o endpoint conforme necessário
    if (response.ok) {
