...
function loadWeeks() {
  ...
  weeks.forEach((week, index) => {
    ...
    toggleStatusButton.addEventListener("click", () => {
      ...
      const newValue = toggleStatusButton.textContent.includes("Sim")
        ? "sim"
        : "nao";
      selects.forEach((select) => (select.value = newValue));
      toggleStatusButton.textContent = `Alterar Todos para ${
        newValue === "sim" ? "Não" : "Sim"
      }`;
    });
    ...
    bulkRegisterButton.addEventListener("click", async () => {
      ...
      for (const row of rows) {
        ...
        try {
          const success = await saveAtraso(
            employeeId,
            week,
            select.value,
            row,
...
async function saveAtraso(
  ...
  try {
    const response = await fetch(
      "http://172.16.222.65:8080/atendentes/update/atrasos",
      {
        method: "PATCH",
        headers: {
